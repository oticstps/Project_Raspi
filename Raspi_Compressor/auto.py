...
FLOAT32_REGISTERS = [
    ...
    (3003, "current_c"),
    (3005, "current_n"),
    # Register 3007 (current_g) dihapus dari pembacaan
    (3009, "current_avg"),
    (3019, "voltage_ab"),
    (3021, "voltage_bc"),
    (3023, "voltage_ca"),
...
def get_current_table_name():
    ...
    now = datetime.now()
    month = now.strftime("%b").lower()  # e.g., 'jan', 'feb', etc.
...
def create_table_if_not_exists(table_name):
    ...
    CREATE TABLE IF NOT EXISTS `{table_name}` (
        ...
        `current_c` FLOAT,
        `current_n` FLOAT,
        `current_g` FLOAT,  -- Tetap ada di tabel, tapi tidak dibaca
        `current_avg` FLOAT,
        `voltage_ab` FLOAT,
        `voltage_bc` FLOAT,
        `voltage_ca` FLOAT,
...
def fetch_modbus_data():
    ...
    # Read float32 registers (tanpa current_g)
...
def insert_data_into_db(data):
    ...
    # Tambahkan current_g sebagai NULL secara eksplisit
    # Karena tidak dibaca dari modbus tapi tetap ada di tabel
...
def main():
    try:
        ...
        client.close()
        db_connection.close()
        print("Connections closed. Exiting program.")
